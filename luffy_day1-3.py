# Day 1
# 匹配成绩的⼩程序，成绩有ABCDE 5个等级，与分数的对应关系如下
'''
A 90-100
B 80-89
C 60-79
D 40-59
E 0-39
要求⽤户输⼊入0-100的数字后，你能正确打印他的对应成绩等级，⽐比如输⼊入的是75，则打印C
'''
...
三等奖30名，避孕套一盒
规则：
1. 共抽3次，第一次抽3等奖，第2次抽2等奖，第3次压抽1等奖
2. 每个员⼯限中奖⼀一次，不能重复
解题思路：
1. ⽣成一个员⼯列表，用random模块从⾥面取随机值
2. 取完值之后，立刻从员工大列表⾥把中奖人删掉，即可防止其再次中奖
'''
import random
...
result = {}
for x in delivery_order:
    if x[1][2] == '市' or x[1][2] == '省' or x[1][2] == '古' or x[1][2] == '江':
        if x[1][:3:] not in result:
            result[x[1][:3:]] = []
        result[x[1][:3:]].append(x)
    elif x[1][1] == '疆' or x[1][1] == '夏':
        if x[1][:2:] not in result:
            result[x[1][:2:]] = []
        result[x[1][:2:]].append(x)
# 格式化输出
print('{')
for x in result:
    print('\t' + '\"' + x + '\":[')
